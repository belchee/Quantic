    ...
    <footer className="bg-gray-900 text-white border-t border-gray-800">
      <div className="max-w-7xl mx-auto px-5 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1">
            <Link href="/" className="inline-block mb-6">
              <Image src="/quantic-logo-light.svg" height={40} width={140} alt="Quantic" unoptimized />
            </Link>
            <p className="text-gray-400 text-sm leading-relaxed mb-6">
              {tr("footer_tagline")}
            </p>
            <a href="https://wa.me/38345460460" target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#25D366]/10 border border-[#25D366]/30 text-[#25D366] text-sm font-medium hover:bg-[#25D366]/20 transition-colors">
          ...
          {/* Company */}
          <div>
            <h3 className="text-gray-400 font-semibold text-xs mb-5 uppercase tracking-widest">{tr("footer_company")}</h3>
          ...
          {/* Services */}
          <div>
            <h3 className="text-gray-400 font-semibold text-xs mb-5 uppercase tracking-widest">{tr("footer_services")}</h3>
            <ul className="space-y-3">
              {serviceLinks.map((l) => (
                <li key={l.href}>
                  <Link href={l.href} className="text-gray-400 text-sm hover:text-white transition-colors">{tr(l.key)}</Link>
          ...
          {/* Contact */}
          <div>
            <h3 className="text-gray-400 font-semibold text-xs mb-5 uppercase tracking-widest">{tr("footer_contact")}</h3>
        ...
        <div className="mt-14 pt-8 border-t border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <p className="text-gray-500 text-xs">{tr("footer_tagline")}</p>
        </div>
      </div>
    </footer>
  );
